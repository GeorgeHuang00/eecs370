comb	lw		0		6		pos1	r6 = 1
		sw		5		7		Stack	save return address on stack
		add		5		6		5		increment stack pointer
		sw		5		1		Stack	save input n on stack
		add		5		6		5		increment stack pointer
		sw		5		2		Stack	save input r on stack
		add		5		6		5		increment stack pointer
base	beq		0		2		ret1	if (r == 0)	return 1
		beq		1		2		ret1	if (n == r) return 1
		beq		6		2		retn	if (r == 1)	return n
		lw		0		6		neg1	r6 = -1
		add		1		6		4		r4 = n - 1
		beq		4		2		retn	if (r == n - 1)	return n
recur	add		1		6		1		n = n - 1
		lw		0		4		Caddr
		jalr	4		7				call the first recursion on (n-1, r)
		lw		0		6		pos1	r6 = 1
		sw		5		3		Stack	save the first return value on stack
		add		5		6		5		increment stack pointer
		lw		0		6		neg1	r6 = -1
		add		2		6		2		r = r - 1
		lw		0		4		Caddr
		jalr	4		7				call the second recursion on (n-1, r-1)
		lw		0		6		pos1	r6 = 1
		sw		5		3		Stack	save the second return value on stack
		add		5		6		5		increment stack pointer
sum		lw		0		6		neg1	r6 = -1
		add		5		6		5		decrement stack pointer
		lw		5		4		Stack	r4 = combination(n-1, r-1)
		add		5		6		5		decrement stack pointer
		lw		5		3		Stack	r3 = combination(n-1, r)
		add		3		4		3		r3 = r3 + r4
		beq		0		0		end		go to the end and return
ret1	lw		0		3		pos1	r3 = 1
		beq		0		0		end
retn	add		0		1		3		r3 = n
end		lw		0		6		neg1	r6 = -1
		add		5		6		5		decrement stack pointer
		lw		5		2		Stack	recover r
		add		5		6		5		decrement stack pointer
		lw		5		1		Stack	recover n
		add		5		6		5		decrement stack pointer
		lw		5		7		Stack	recover return address
		jalr	7		4				return
pos1	.fill	1
neg1	.fill	-1
Caddr	.fill	comb
